...
if (!environment.success) throw new Error("DATABASE_URL is required for local seeds.");
...
const applied = await runSeeds(environment.data.DATABASE_URL);
console.log(`Local seeds applied: ${applied.length === 0 ? "none (all current)" : applied.join(", ")}`);
