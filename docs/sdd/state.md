...
- Responsive/adaptivity audit pass (post-030, quick-fix branch → PR): added `overflow-x: clip` on `html` (kills the horizontal micro-scroll caused by the `100vw` `.bleed` full-bleed sections whenever a scrollbar is present), made the catering honeypot input zero-size (it was overflowing the viewport at 320–360 with its native width), and raised the AI chat dialog above the fixed mobile action bar (`bottom-20 sm:bottom-4`). Verified in Playwright across `/de*` public pages at 320/360/768/1280: no document overflow outside the intentional clipped hero bleeds, no horizontal scrolling.
