...
export default async function MobileActionBar(): Promise<React.ReactElement> {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-40 border-t border-lime/20 bg-brand-deep sm:hidden"
    >
      <ul className="grid grid-cols-5 px-1 py-1">
        {actions.map((action) => (
          <li key={action.href}>
            <Link
              href={action.href}
              className="flex min-h-11 items-center justify-center px-1 text-center text-xs font-medium text-cream/85 transition-colors duration-300 hover:text-lime"
            >
              {action.label}
            </Link>
          </li>
