...
export default async function SiteHeader(): Promise<React.ReactElement> {
  ...
  return (
    <header>
      <div data-logo-bar className="sticky top-0 z-40 border-b border-brand-deep/10 bg-paper/88 shadow-sm shadow-brand-deep/5 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-8">
          <Link
            href="/"
            aria-label={brandTranslations("name")}
            className="logo-lockup flex items-center gap-3 transition-opacity hover:opacity-90"
          >
            <KalynaLogo />
